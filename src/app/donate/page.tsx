...
import { DollarSign, Heart, Repeat, Shield, Info, Gift, EyeOff, CreditCard, Wallet, Landmark, Banknote, User, Mail, Lock, CheckCircle2, Loader2 } from "lucide-react";
...
export default function Donate() {
  ...
  const proceedToCheckout = async () => {
    ...
    try {
      ...
      // Use embedded mode for one-time payments, redirect for recurring
      const useEmbedded = !isRecurring;
      ...
      const { data, error } = await supabase.functions.invoke("create-payment-intent", {
        body: {
          ...
          mode: useEmbedded ? "embedded" : "redirect",
        },
        headers: authToken ? {
      ...
      if (data?.client_secret && useEmbedded) {
        // Embedded mode: show payment form in-page
        if (data.claim_token) {
        ...
        // Redirect mode for recurring payments
        if (data.claim_token) {
          sessionStorage.setItem("donation_claim_token", data.claim_token);
        }
  ...
  return (
    <div className="space-y-8 max-w-2xl mx-auto px-4 pb-12">
      ...
      {paymentSuccess && (
        <Card className="border-border/50">
          <CardContent className="p-8 text-center space-y-6">
            <div className="w-20 h-20 mx-auto rounded-full bg-gradient-to-br from-green-100 to-green-200 flex items-center justify-center">
      ...
      {/* Main Donation Form - hide when showing payment or success */}
